...
Focus: Understanding GPU thread indexing, memory coalescing, and basic arithmetic.
...
| 07 | Sigmoid | Numerical stability for $1 / (1 + e^{-x})$ |
| 08 | GeLU | GELU Approximation Kernel |
| 09 | SiLU (Swish) | $x \cdot \text{sigmoid}(x)$ |
| 10 | Tanh | Hyperbolic Tangent |
| 11 | Boundary Handling | Handling non-aligned sizes ($N \neq \text{block\_size} \times K$) |
| 12 | 2D Strided Load | Row-major/Col-major layout conversion |
| 13 | Vector Mean | 1D Vector average |
| 14 | L2 Norm | $\sqrt{\sum x^2}$ |
| 15 | Clamp/Clip | Limit $x \in [min, max]$ |
...
Focus: Shared memory, synchronization, and communication between threads.
...
| 28 | Variance | Row-wise $\text{Var}(x)$ |
...
| 34 | Log-Softmax | $\log(\text{softmax}(x))$ |
...
| 39 | Min-Max Normalizer | $x_{norm} = (x - min) / (max - min)$ |
| 40 | Dot Product | Vector-Vector inner product reduction |



## 🟠 Phase 3: Normalization & Memory Layout (Day 41–60)
Focus: Combining reductions with element-wise operations and advanced layouts.

| Day | Topic | LeetGPU Task / Implementation Goal |
|:---:|:---|:---|
| 41 | LayerNorm (FWD) | Normalization based on $\mu, \sigma$ |
...
| 47 | LayerNorm (Weight BWD)| $d\gamma, d\beta$ weight gradient calculation |
...
| 51 | Padding | Papping arbitrary matrices for Tiled ops |
| 52 | Unpadding | Removing padding and extracting data |
| 53 | Concatenation | Joining two tensors along a specific dim |
...
Focus: Tiling, L2 cache blocking, and tensor core utilization.
...
| 71 | GEMM + Activation | $C = \text{ReLU}(AB)$ Fusion |
...
Focus: Implementing state-of-the-art kernels used in modern Transformers.
...
| 84 | Scaled Dot-Product| $QK^T / \sqrt{d_k}$ Score calculation |
